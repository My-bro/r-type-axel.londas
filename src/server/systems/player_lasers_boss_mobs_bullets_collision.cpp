...
** rtype
** File description:
** player_lasers_boss_mobs_bullets_collision
*/
...
{
    for (std::size_t i = 0; i < player_lasers.size() && i < player_positions.size() && i < player_hitboxes.size() && i < player_damages.size(); i++) {
        auto &player_laser = player_lasers[i];
        auto &player_pos = player_positions[i];
        auto &player_hitbox = player_hitboxes[i];
        auto &player_damage = player_damages[i];
        if (player_laser && player_pos && player_hitbox && player_damage) {
